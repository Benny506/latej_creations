...
const GlobalNavbar = () => {
  ...
  const retailCatalogs = activeCatalogs.filter(c => c.type === 'retail')
  const wholesaleCatalogs = activeCatalogs.filter(c => c.type === 'wholesale')
        ...
        <Container>
          ...
          <div className="d-none d-lg-flex flex-grow-1 justify-content-center">
            <Nav className="gap-4 align-items-center">
              ...
              {/* Desktop Dropdown for Collections */}
              <NavDropdown
                title={
                  <span className={`tiny text-uppercase fw-bold tracking-widest transition-all ${isActive('/catalog') ? 'text-primary' : 'text-main opacity-75'}`}>
                    Catalogs
                  </span>
                }
                ...
                className="aso-nav-dropdown d-flex align-items-center"
              >
                {retailCatalogs.length > 0 && <NavDropdown.Header className="tiny text-uppercase fw-bold opacity-50 tracking-widest">Retail</NavDropdown.Header>}
                {retailCatalogs.map(catalog => (
                  <NavDropdown.Item key={catalog.id} as={Link} to={`/shop?category=${catalog.id}`} className="tiny text-uppercase fw-bold py-2 px-4">
                    {catalog.name}
                  </NavDropdown.Item>
                ))}
                ...
                {wholesaleCatalogs.length > 0 && (
                  <>
                    <NavDropdown.Divider />
                    <NavDropdown.Header className="tiny text-uppercase fw-bold opacity-50 tracking-widest">Wholesale</NavDropdown.Header>
                  </>
                )}
                {wholesaleCatalogs.map(catalog => (
                  <NavDropdown.Item key={catalog.id} as={Link} to={`/wholesale?category=${catalog.id}`} className="tiny text-uppercase fw-bold py-2 px-4">
                    {catalog.name}
                  </NavDropdown.Item>
                ))}

                <NavDropdown.Divider />
                <NavDropdown.Item as={Link} to="/catalog" className="tiny text-uppercase fw-bold py-3 px-4 text-primary bg-primary-light">
      ...
      >
        ...
        <Offcanvas.Body className="p-4">
          <div className="d-flex flex-column gap-2">
            ...
            <div className="p-3 bg-white rounded-5 shadow-sm border border-light my-2">
              <span className="tiny text-uppercase fw-bold opacity-25 tracking-widest d-block mb-3" style={{ fontSize: '0.6rem' }}>Collections</span>
              <div className="d-flex flex-column gap-2">
                {retailCatalogs.length > 0 && <span className="tiny text-uppercase fw-bold opacity-50 tracking-widest ps-2 pt-2" style={{ fontSize: '0.6rem' }}>Retail</span>}
                {retailCatalogs.map(catalog => (
                  <Link key={catalog.id} to={`/shop?category=${catalog.id}`} onClick={handleClose} className={`d-flex align-items-center justify-content-between p-3 rounded-4 text-decoration-none transition-all bg-light text-main opacity-75`}>
                    <span className="fw-bold tiny text-uppercase tracking-widest">{catalog.name}</span>
                    <ChevronRight size={14} />
                  </Link>
                ))}

                {wholesaleCatalogs.length > 0 && <span className="tiny text-uppercase fw-bold opacity-50 tracking-widest ps-2 pt-2" style={{ fontSize: '0.6rem' }}>Wholesale</span>}
                {wholesaleCatalogs.map(catalog => (
                  <Link key={catalog.id} to={`/wholesale?category=${catalog.id}`} onClick={handleClose} className={`d-flex align-items-center justify-content-between p-3 rounded-4 text-decoration-none transition-all bg-light text-main opacity-75`}>
                    <span className="fw-bold tiny text-uppercase tracking-widest">{catalog.name}</span>
                    <ChevronRight size={14} />
                  </Link>
                ))}
                ...
                <Link to="/catalog" onClick={handleClose} className={`d-flex align-items-center justify-content-between p-3 rounded-4 text-decoration-none transition-all bg-primary-light text-primary`}>
                  <span className="fw-bold tiny text-uppercase tracking-widest">View All Collections</span>
                  <ChevronRight size={14} />
                </Link>
              </div>
            </div>
